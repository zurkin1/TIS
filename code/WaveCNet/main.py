...
parser.add_argument('--data', metavar = 'DIR', default = '~rimonshy/MS_rimon/data/SKCM/SKCM-polygon_500_40X', #/home/dsi/zurkin/data/SKCM-polygon_20X',
                    help = 'path to dataset')
#parser.add_argument('--data_test', metavar = 'DIR', default = '/home/dsi/zurkin/data/SKCM-polygon_20X_test',
#                    help = 'path to dataset')
parser.add_argument('--root_save', metavar = 'DIR', default = './',
                    help = 'path to save the trained model and daily record')
parser.add_argument('-a', '--arch', metavar = 'ARCH', default = 'resnet18_dwt',
                    help = 'model architecture: ' +
                           ' | '.join(model_names) +
                           ' (default: resnet18)')
parser.add_argument('-j', '--workers', default = 160, type = int, metavar = 'N',
...
parser.add_argument('--weights', default = "./weights/resnet18_dwt/resnet18_dwt_haar_256_best.pth.tar", type = str, metavar = 'PATH',
...
def main_worker(gpu, args, printer):
    ...
    optimizer = torch.optim.Adam(model.parameters(), args.lr, weight_decay = args.weight_decay) # momentum = args.momentum, weight_decay = args.weight_decay)
    ...
    #    model.module.fc = nn.Linear(512, 2, bias=True)
    #    model = torch.nn.parallel.DataParallel(model.cuda(), device_ids = args.gpu, output_device = args.gpu_out)
...
def train(train_loader, model, criterion, optimizer, epoch, args, printer, start_time):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            #input = input.to(args.gpu[0], non_blocking=True)
            input = input.cuda()
        target = target.cuda()
        model = model.cuda()
        ...
        if i % args.print_freq == 0:
            ...
            printer.pprint('Epoch: [{0}][{1}/{2}]\t' #  'Total_time {3}\t' 'Batch time avg {batch_time.avg:.3f}\t' 'Data time avg {data_time.avg:.1f}\t'
                           'Loss avg {loss.avg:.2f}\t'
                           'Acc avg {top1.avg:.2f}\t'
                           'lr {lr}'.format(
...
def validate(val_loader, model, criterion, args, printer, start_time = None):
    ...
    with torch.no_grad():
        ...
        printer.pprint(f' * Val acc {top1.avg:.2f}')
...
def accuracy(output, target):
    with torch.no_grad():
        val, ind = output.sigmoid().max(1)
        res = torch.sum(ind == target)
        batch_size = target.size(0)
        #correct = pred.eq(target.view(1, -1).expand_as(pred))
        res = (res * 100.0 / batch_size)
        return res
