...
batch_tfms3 = [*aug_transforms(do_flip=True, flip_vert=True, max_rotate=180.0, max_lighting=0.4, p_affine=0.7, p_lighting=0.7)]
src = ImageDataLoaders.from_folder(path+'train/', valid_pct=0.05, batch_tfms=batch_tfms3).normalize() #bs=64, item_tfms=item_tfms, 
...
learn = cnn_learner(src, resnet50, path=path+'train/', metrics=[accuracy, Precision(average='weighted')], loss_func=loss_func) #cut=-2, splitter=_resnet_split, loss_func=F.binary_cross_entropy_with_logits, metrics=[f1score_multi]
...
learn.fine_tune(20, base_lr=lr, freeze_epochs=1) #, cbs=[EarlyStoppingCallback(patience=3), SaveModelCallback(monitor='accuracy')])
